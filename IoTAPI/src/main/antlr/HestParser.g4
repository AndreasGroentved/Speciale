...
rulee: 'rule' '{' ruleContent* '}';
ruleContent: config|variable|condition|device|output|run;
config: 'config' time;
...
run: 'run' '{' runContent+ '}';
runContent: condition (output|varpath)+ | output|varpath+;
...
dataset: 'dataset' '{' datasetContent* '}';
tag: 'tag' '=' STRINGLIT;
...
datasetContent: tag | header | name | format | formatType | variable;
